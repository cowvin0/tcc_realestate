...
A transformação logarítmica $\log(1 + x)$ foi aplicada para estabilizar a variância e tornar a distribuição dos regressores mais simétrica. Embora, a transformação $\log(1 + x)$ tenha sido aplicada para a maior parte das variáveis numéricas, ela não foi aplicada para uma única variável, a que representa o produto entre as coordenadas geográficas e o valor de aluguel, nessa foi aplicada a transformação de Yeo-Johnson [@yeo], que contém a vantagem de ser aplicável para dados que contém valores negativos e 0. A transformação de Yeo-Johnson é definida da seguinte forma:
