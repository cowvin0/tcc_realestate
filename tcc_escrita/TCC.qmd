...
\ \ \ No algoritmo de Bagging cada árvore era construída e ajustada utilizando uma amostra bootstrap. Por fim, era necessário tomar um agregado $\varphi_B$ de todas as $B$ árvores para criar um único estimador. O Boosting Trees funciona de forma similar, no entanto, cada árvore é construída utilizado a informação prévia de outras árvores, ou seja, são construídas sequenciamente. A informação utilizada
