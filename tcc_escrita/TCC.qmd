...
\ \ \ O algoritmo de Gradient Boosting é semelhante ao Boosting, mas com algumas diferenças. Ele constrói modelos aditivos ajustando sequencialmente funções base aos pseudos-resíduos em cada iteração, que, por sua vez, correspondem aos gradientes da função de perda [@friedman2002stochastic]. Esses podem gradientes indicam a direção na qual a função perda diminui mais rapidamente, orientando o ajuste dos modelos seguintes.
...
<!-- \ \ \ O algoritmo de Gradient Boosting é semelhante ao de Boosting, mas com diferenças mínimas. Ele constrói modelos aditivos ajustando sequencialmente funções bases aos pseudos-resíduos a cada iteração, que correspondem aos gradientes da função perda [@friedman2002stochastic]. Esses gradientes indicam a direção na qual a função perda diminui. Neste trabalho, foram utilizadas diferentes implementações de Gradient Boosting. No entanto, todas empregam o Gradient Boosting com árvores de regressão, com algumas modificações para a construção das árvores ou para melhorar a eficiência do algoritmo existente. Assim, o algoritmo a ser explicado será o Gradient Tree Boosting. O @algo-gradboos demonstra a sua construção. -->
...
$$
onde $N_{jm}$ denota o número de observações na região $R_{jm}$. Os pseudo-resíduos $r_{im}$​ representam os gradientes da função de perda $L$, conforme expressos na linha 2(a) do @algo-gradboos, e indicam a direção de maior diminuição de $L$.

\vspace{12pt}

Assim, cada árvore de regressão é ajustada aos novos pseudo-resíduos, com o objetivo de minimizar o erro residual acumulado pelas árvores anteriores. O estimador é, então, atualizado separadamente em cada região $R_{jm}$​, conforme descrito a seguir:
...
$$
em que $\gamma_{jm} = \arg \min_{\gamma} \sum_{x_i \in R_{jm}} L\left(y_i, f_{m - 1}\left(x_i\right) + \gamma\right)$ representa a constante ótima para cada região. O parâmetro $\lambda$, assim como no algoritmo de boosting, é o hiperparâmetro de shrinkage, utilizado para controlar a taxa de aprendizado. Valores menores de $\lambda$ exigem um número maior de iterações $M$.
