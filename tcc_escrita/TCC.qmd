...
Isso significa que, à medida que o número de árvores $B$ aumenta, o segundo termo da soma se torna menos significativo. Portanto, os benefícios da agregação proporcionados pelo algoritmo de Bagging são limitados pela correlação entre as árvores [@hastie2009elements]. Mesmo com o aumento do número de árvores no Bagging, a correlação entre elas impede que as estimações individuais sejam completamente independentes, resultando em menor diminuição da variância da média das previsões do que seria esperado se as árvores fossem totalmente independentes. Ou seja, as árvores geradas pelo Bagging tendem a ser bastante semelhantes, uma vez que as variáveis independentes selecionadas para o particionamento são escolhidas com base na redução do quadrado dos resíduos em cada região. Uma maneira de melhorar o algoritmo de Bagging é por meio do Random Forest, que será descrito a seguir.
