...
Antes de se iniciar a raspagem dos dados com o [**Scrapy**](https://docs.scrapy.org/en/latest), é necessário que se inicie um projeto usando o seu CLI. Para isso, é utilizado o comando `scrapy startproject <nome_do_projeto>`. Esse comando irá criar um diretório com todos os arquivos e configurações iniciais necessárias para se iniciar a extração de dados com o [**Scrapy**](https://docs.scrapy.org/en/latest). Por exemplo, suponha que o comando `scrapy startproject scraping_examples` tenha sido executado. Após a sua execução, será criado o diretório com a seguinte estrutura:
