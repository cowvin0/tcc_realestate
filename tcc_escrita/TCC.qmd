...
Todos os modelos e bibliotecas usados neste trabalho seguem a API do scikit-learn. Nessa API, o hiperparâmetro que define a quantidade de árvores é denominado `n_estimators`, o de profundidade das árvores é `max_depth`, e o de taxa de aprendizado é `learning_rate`. Com a configuração da função objetivo na biblioteca Optuna, foi possível encontrar o ponto ótimo desses hiperparâmetros para cada modelo.
...
# contour = fig_rf_contour.get_children()[0]

# Adicionar a barra de cores manualmente usando plt.colorbar()
# cbar = plt.colorbar(contour, ax=fig_rf_contour)
# cbar.set_label("Nova Legenda da Barra de Cores", rotation=270, labelpad=15)


plt.show()
```

Resultados da Tunagem do modelo Random Forest.
:::

### Tunagem do Gradient Boosting

::: {#fig-gdt_param layout-nrow=2}
