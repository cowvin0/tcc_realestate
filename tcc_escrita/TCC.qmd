...
\ \ \ Como os algoritmos utilizados nesse trabalho são baseados em árvore de decisão ou fazem uso de árvore de decisão, como o Random Forest, Gradient Boosting e suas variações, os parâmetros escolhidos para otimização foram bastante semelhante. Dessa forma, foram tunados hiperparâmetros como a quantidade de árvores, a profundidade da árvore para representar a complexidade das relações presentes nos dados. Por fim, como alguns dos algoritmos são baseados no Gradient Boosting, foi também tunado o hiperparâmetro da taxa de aprendizado.

\vspace{12pt}

Todos os modelos e bibliotecas utilizadas nesse trabalho se baseiam no funcionamento da API do scikit-learn. A API do scikit-learn define o hiperparâmetro da quantidade de árvores como `n_estimators`, o de profundidade da árvore como `max_depth` e o de taxa de aprendizado como `learning_rate`. Dessa forma, a partir da configuração da função objetivo pela biblioteca optuna, a procura pela quantidade ótima de árvores variou entre 50 e 1500, o de profundidade da árvore foi de 3 a 500 e o de taxa de aprendizado foi de $1 \cdot 10^{-4}$ a $1 \cdot 10^{-1}$.
...
# optuna.visualization.plot_edf([study1, study2, study3, study4])
# optuna.visualization.plot_optimization_history([study1, study2, study3, study4])
...
optuna.visualization.matplotlib.plot_contour(study2)
optuna.visualization.matplotlib.plot_contour(study3)
optuna.visualization.matplotlib.plot_contour(study4)
```

```{python}
optuna.visualization.matplotlib.plot_slice(study1)
optuna.visualization.matplotlib.plot_slice(study2)
optuna.visualization.matplotlib.plot_slice(study3)
optuna.visualization.matplotlib.plot_slice(study4)
```
