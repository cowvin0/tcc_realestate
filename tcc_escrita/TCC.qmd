...
\ \ \ No caso de uma árvore de decisão para classificassão, a única coisa que mudaria seria o critério para divisão dos nós e a poda da árvore. Além disso, a previsão em um nó $j$ de uma região $R_j$ com $N_j$ observações será simplesmente a classe majoritária. Assim, tem-se

$$
\hat{p}_{jk} = \frac{1}{N_j}\sum_{x_i \in R_j} I\left(y_i = k\right)
$$
a proporção de observações da classe $k$ no nó $j$. Dessa forma, são classificadas as observações no nó $j$ para a classe $k\left(j\right) = \arg \max_{k} \hat{p}_{jk}$, que é a moda no nó $j$. Para se fazer a divisão dos nós no caso da regressão foi utilizado o erro quadrático como medida de impureza. Para a classificação, algumas medidas comuns para $Q_m\left(T\right)$ é o misclassification erro, gini index ou cross-entropy.
...
\ \ \ Matematicamente, o Stacking define predições $\hat{f}_m^{-i}\left(x\right)$ em x, utilizando o modelo estatístico ou algoritmo $m$, aplicado ao conjunto de treinamento com a $i-ésima$ observação removida [@hastie2009elements]. Assim, os peso são estimados de maneira a minimizar o erro de previsão combinado, dado pela seguinte expressão

$$
\hat{w}^{st} = \arg \min_{w} \sum^{N}_{i = 1} \left[y_i - \sum^{M}_{m = 1} w_m f^{-i}_m\left(x_i\right)\right]^2
$$
A previsão final dos modelos empilhados é $\sum_{m} \hat{w}_m^{st} \hat{f}_m\left(x\right)$. Portanto, ao invés de escolher um único modelo, o método de stacking combina eles com pesos estimados ótimos, o que acaba melhorando a performance preditiva, mas prejudica a interpretabilidade.
