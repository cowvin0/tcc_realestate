...
\ \ \ A validação cruzada por K-Fold utiliza parte dos dados para ajustar o modelo e outra parte para testá-lo. Dessa forma, os dados são divididos em K partes, sendo que uma dalas será apenas para testar o modelo. O modelo é ajustado para os outros $K-1$ partes dos dados e é testado na $K\text{-ésima}$ parte para calcular o erro de predição. Isso é feito $k = 1, 2, \dots, K$ vezes e os $K$ erros de estimação são combinados no final. Assim, o erro de predição estimado pela validação cruzada é dado por:
...
def plot_cv_indices(cv, X, ax, n_splits, lw=10):
    ...
    # Adjust the axis labels and limits
