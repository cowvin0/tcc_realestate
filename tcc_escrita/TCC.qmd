...
\ \ \ Foi realizada também a análise das distribuições das variáveis numéricas da base de dados, como pode ser visto na @fig-dists. Nessa figura, a distriuição das variáveis forma analisadas por tipo de imóvel. Foi possível observar que a maioria das distriuições apresentam assimetria positiva. As distribuções apresentam caudas longas à direita, indicando a presenta de outliers, principalmente quando são observados os terrenos.
...
plt.legend(loc='upper center', ncol=7, bbox_to_anchor=(.005, -.00001))
```
