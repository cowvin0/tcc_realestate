...
Na sessão a seguir, apresenta-se um exemplo de como realizar web scraping em **R** e **Python**. Para isso, utilizam-se os pacotes mencionados para a linguagem **R** e, no caso de **Python**, dá-se preferência ao [**Scrapy**](https://docs.scrapy.org/en/latest), a principal biblioteca empregada para esse fim. É importante ressaltar que os códigos desenvolvidos para a raspagem de dados podem deixar de funcionar ao longo do tempo devido a possíveis mudanças na estrutura dos sites. Assim, eventuais ajustes no código poderão ser necessários.


### Exemplo de web scraping em Python

\ \ \ Os exemplos de raspagem de dados em **R** e **Python** utilizarão o mesmo site. O objetivo é extrair informações sobre todos os pacotes de **R**, tanto os ordenados por data de publicação quanto os organizados por nome, disponíveis em: <https://cran.r-project.org/web/packages/index.html>. A partir desses dados, serão gerados dois arquivos `.csv` contendo essas informações.
...
::: {layout-ncol=2}
![O HTML conterá todo conteúdo contido nessa página.](includes/page_contents.png){#fig-page_contents}

![Exemplo do uso das ferramentas de desenvolvedor do Firefox para inspencionar conteúdo da página.](includes/page_content_devtools.png){#fig-page_contents_devtools}
:::

Com a execução do comando anterior, o conteúdo da página na @fig-page_contents será gerado e o conteúdo contido em seu HTML poderá ser extraído. Se o `fetch` deu certo, a saída do `response.status` no shell deverá ser 200. Para visualizar o conteúdo do HTML, basta executar `response.body`, como pode ser visto a seguir, mostrando apenas uma parte de seu conteúdo:
...
<p> Currently, the CRAN package repository features 22127 available packages. </p>
...
```
