...
\ \ \ O algoritmo de Gradient Boosting aplicado a árvores de decisão é um dos mais eficientes e amplamente utilizados, sendo frequentemente vencedor em competições de modelagem estatística. Existem diversas variações desse algoritmo, cada uma com seus próprios motivos de existênica. Esses motivos incluem a busca por maior eficiência computacional, facilidade de uso, adição de recursos (como no caso da implementação CatBoost, que suporta dados categóricos) e até mesmo maior flexibilidade.

\vspace{12pt}

Além da definição anterior de Gradient Boosting aplicado a árvores de decisão dada anteriormente, esse trabalho também utilizou como outras implementações de Gradient Boosting, como o Light Gradeint Boosting e o Extreme Gradient Boosting, que partem da mesma premissa do Gradient Boosting, mas adicionam outras funcionalidades e tem uma implementação mais computacionalmente eficiente. Dessa forma, esta sessão irá definir as diferentes implementações que foram utilizadas e evidenciará as suas principais característica.
