...
![](includes/page_content_devtools_bigger.png){#fig-page_contents_devtools}
...
Em relação ao algoritmo de Gradient Boosting, seu ajuste consegue explicar $87,32712%$ da variável dependente, um valor superior ao obtido com o ajuste do algoritmo de Random Forest, embora a diferença não seja muito grande. Para o MAPE, o valor encontrado foi de $0,01333$, indicando que os valores estimados dos imóveis estão, em média, $1,333%$ dos seus valores observados. A relação entre os valores estimados e os observados pode ser visualizada na @fig-preds_gdt.
...
<!-- Embora o usuário tenha a opção de se selecionar as coordenadas diretamente e informações de aluguel diretamente pelo mapa, o usuário ainda terá a opção de passar diretamente os valores nos blocos dessas informações. Além disso, como inicialmente foi utilizado o KNN como método de imputação de valores ausentes, o usuário não é obrigado a passar nenhum dos outros valores além do tipo de imóvel. Dessa forma, caso não se tenha as informações completas do imóvel, é possível ainda assim realizar as previsões, basta deixar todos ou alguns dos blocos vazios. -->
