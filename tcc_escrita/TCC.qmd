...
\ \ \ A história do Brasil, assim como a de muitos outros países, foi marcada por uma predominância rural durante grande parte de sua trajetória. Em 1940, apenas 31% da população vivia em áreas urbanas. Contudo, esse cenário mudou rapidamente, e, em 1970, mais da metade da população já residia em cidades [@wagner1980urbanization]. Esse processo de urbanização acelerado e desordenado resultou em um crescimento desorganizado das grandes metrópoles, agravando problemas urbanos, como o déficit habitacional. Com o aumento das regiões urbanizadas, a demanda por habitação tornou-se cada vez maior, exigindo medidas para mitigar o déficit habitacional. Nesse contexto, foi instituída a Lei nº 4.380, de 21 de agosto de 1964, que criou pela primeira vez no país um mecanismo de crédito habitacional. Essa legislação foi crucial para articular a oferta e a demanda de recursos destinados ao setor habitacional e resultou na criação do Sistema Financeiro de Habitação (SFH), que introduziu inovações como a correção monetária, o Banco Nacional da Habitação (BNH) e as Sociedades de Crédito Imobiliário (SCI). Embora o SFH tenha obtido muitos sucessos, o aumento da inflação na década de 1980 impactou seu funcionamento, agravado por medidas adotadas pelos governos para combater a inflação. Assim, com a experiência acumulada pelo SFH, foi criado o Sistema de Financiamento Imobiliário (SFI), que não substituiu o SFH, mas marcou o início de um modelo moderno de financiamento habitacional no Brasil.
...
::: {#fig-abecip-inve layout-ncol=2}

![Unidades habitacionais financiadas pelo SFH, 1964-2005.](includes/hab_sfh.png){#fig-hab_sfh}

![Evolução do investimento habitacional.](includes/financ_hab.png){#fig-fincanc_hab}

Fonte: @abecip_credito2007
:::
...
O método ICE é equivalente ao Partial Dependence Plot (PDP), com a diferença que o ICE faz o mesmo considerando uma linha para cada instância, o que permite observar o comportamento das predições em relação a variação das variáveis. Por outro lado, o PDP é um método mais global, ele vai ser a média de todas as linhas presentes em um gráfico do método ICE. Dessa forma, o PDP, para regressão, é definido da seguinte forma:
...
O SHAP estabelece uma conexão entre o conceito de valores de Shapley e o método LIME, previamente definido. O modelo explicativo do SHAP, denotado por $g$, é definido como uma função linear de variáveis binárias e está em função valores de Shapley, conforme apresentado abaixo:

$$
g\left(z^{'}\right) = \phi_0 + \sum_{j = 1}^{M} \phi_{j} z_{j}^{'}
$$
onde $g$ é o modelo explicativo, $z^{'} \in \{0, 1\}^{M}$ é o vetor de coalizão, representando a presença $\left(z^{'}_j = 1\right)$ ou ausência $\left(z^{'}_j = 0\right)$ de cada covariável e $\phi_j$ denota os valores Shapley.
...
Para analisar a influência das variáveis na predição dos valores de imóveis, foi utilizado o gráfico de ICE (Individual Conditional Expectation) em conjunto com a linha de PDP (Partial Dependence Plot). Esses gráficos estão representados na @fig-ice_pdp.
