...
Na sessão a seguir, apresenta-se um exemplo de como realizar web scraping em **R** e **Python**. Para isso, utilizam-se os pacotes mencionados para a linguagem **R** e, no caso de **Python**, dá-se preferência ao [**Scrapy**](https://docs.scrapy.org/en/latest), a principal biblioteca empregada para esse fim. Além disso, essa sessão servirá também para mostrar a estruturar de um HTML e o funcionamento dos seletores para selecionar elementos de um HTML. É importante ressaltar que os códigos desenvolvidos para a raspagem de dados podem deixar de funcionar ao longo do tempo devido a possíveis mudanças na estrutura dos sites. Assim, eventuais ajustes no código poderão ser necessários.


### Exemplo de web scraping em Python

\ \ \ Os exemplos de raspagem de dados em **R** e **Python** utilizarão o mesmo site. O objetivo é extrair informações sobre todos os pacotes de **R** ordenados por data de publicação disponíveis em: <https://cran.r-project.org/web/packages/index.html>. A partir desses dados, será gerado um arquivo `.csv` contendo essas informações.

\vspace{12pt}

Para instalar o [**Scrapy**](https://docs.scrapy.org/en/latest) e suas dependências, pode-se instalar diretamente utilizando o [**pip**](https://pip.pypa.io/en/stable/), que é o gerenciador de pacotes para **Python**. Portanto, para isso, basta executar em seu terminal o comando `pip install Scrapy`.

\vspace{12pt}

Após a instalação do [**Scrapy**](https://docs.scrapy.org/en/latest), ficará disponível o uso de seu CLI^[CLI (Command Line Interface) é uma interface de linha de comando que permite a interação com um sistema operacional ou software por meio de comandos digitados em um terminal, sem o uso de interface gráfica.]. O CLI disponibilizará inúmeras funcionalidades para o [**Scrapy**](https://docs.scrapy.org/en/latest), tais como inspencionar o conteúdo da página para encontrar mais facilmente os seletores daquela página em específico e, o mais importante, iniciar um projeto [**Scrapy**](https://docs.scrapy.org/en/latest).

\vspace{12pt}

Antes de se iniciar a raspagem dos dados com o [**Scrapy**](https://docs.scrapy.org/en/latest), é necessário iniciar um projeto utilizando o seu CLI. Para isso, basta executar o comando `scrapy startproject <nome_do_projeto>`. Esse comando criará um diretório com todos os arquivos e configurações iniciais necessários para começar a extração de dados com o [**Scrapy**](https://docs.scrapy.org/en/latest). Por exemplo, ao executar o comando `scrapy startproject scraping_examples`, será criada um diretório com a seguinte estrutura:
...
Com a estrutura do projeto criada, é possível criar o primeiro spider do projeto. Para isso, é utilizado o comando `scrapy genspider <nome_do_spider> <website>`. Neste exemplo, serão raspados informações do site <https://cran.r-project.org/web/packages/index.html>. Ao executar o comando `scrapy genspider packagescraper https://cran.r-project.org/web/packages/index.html`, será gerado um arquivo dentro do diretório spiders com o nome `packagescraper.py`, que terá a seguinte estrutura:
...
A classe `PackagescraperSpider` foi criada a partir do comando executado anteriormente. Essa classe possui três atributos e um método. O atributo `name` nomeia o spider e vai ser utilizado posteriormente para inicializar a raspagem dos dados. O `allowed_domains` é o atributo que indica para o [**Scrapy**](https://docs.scrapy.org/en/latest) que ele deve apenas raspar informações que estão naquele domínio, nesse caso, `cran.r-project.org`. O atributo `start_urls` indica qual deve ser a primeira url a ser raspada. Por último, o método `parse` é executada depois que uma resposta foi obtida após a requisição no site alvo.

\vspace{12pt}

Para extrair dados do conteúdo HTML da página, será necessário utilizar os seletores `CSS` ou `XPath`. Nesse caso, será utilizado o seletor do `CSS`. De forma a facilitar a seleção de conteúdo da página, será utilizado uma das grandes ferramentas do [**Scrapy**](https://docs.scrapy.org/en/latest), que é o [**Scrapy Shell**](https://docs.scrapy.org/en/latest/topics/shell.html). Essa ferramenta ajudará a selecionar as informações que necessárias e até mesmo a debugar o código. Para executar o [**Scrapy Shell**](https://docs.scrapy.org/en/latest/topics/shell.html), basta utilizar o comando `scrapy shell` no terminal. Após a sua execução, aparecerão algumas informações de logs e de configurações gerais que já estão sendo utilizadas no seu projeto. Agora, com o shell ativado, basta trazer todo o conteúdo HTML contido na página de interesse com o seguinte comando: `fetch("https://cran.r-project.org/web/packages/index.html")`.
...
A @fig-page_contents mostra a página que está sendo raspada, enquanto a @fig-page_contents_devtools mostra a mesma página mas com um exemplo do uso das ferramentas de desenvolvedor do Firefox.
:::

Com a execução do comando anterior, o conteúdo da página na @fig-page_contents será gerado e o conteúdo contido em seu HTML poderá ser extraído. Se o `fetch` deu certo, após a execução do comando deverá aparecer um código 200, que é um código de status HTTP que indica que uma requisição foi bem sucedida. A saída a seguir exemplifica esse resultado:
...
Após a execução do comando `fetch`, o conteúdo da página ficará salvo em um objeto `response`, que faz parte nativamente do shell. Por exemplo, caso se deseja obter o conteúdo contido no HTML da página, basta executar `response.body`. É a partir dessa resposta que o conteúdo da página poderá ser analisado e os seletores poderão finalmente ser configurados. No entanto, para facilitar ainda mais esse trabalho, alguns navegadores possuem ferramentas de desenvolvedor, que facilitam a inspeção da estrutura da página, como pode ser visto na @fig-page_contents_devtools.
...
Após inspencionar a página, percebe-se que possuem alguns elementos `<a>` no corpo do HTML e que possuem um href relacionado a eles. Esses href serão necessários para fazer a paginação fazer a coleta dos pacotes ordenados por data de publicação. Para extrair esses href, basta executar em seu terminal o comando `response.css("a[target=_top] ::attr(href)").get()` que faz uso dos seletores `CSS`. Esse comando é usado para extrair apenas o primeiro href de elemento `<a>` que possuem o atributo target="_top". Caso fosse necessário extrair todos os href, bastaria substituir o método `.get` por `.getall()`.

\vspace{12pt}

Com a execução do comando, é extraída uma string que servirá para fazer a paginação, com nome `'available_packages_by_date.html'`. É a partir dessa string que será feita a paginação. O `index.html` que está no atributo `start_urls` deverá ser substituído pela string obtida e seguir para a págian de pacotes ordenados por data de publicação. Agora, com esse href, pode-se utilizar o shell e executar o `fetch` na url <https://cran.r-project.org/web/packages/available_packages_by_date.html> para carregar o conteúdo da página com a tabela de todos os pacotes ordenados por data.

\vspace{12pt}

A nova página contém uma tabela geral de todos os pacotes ordenados por data e contendo a sua descrição. Cada data, nome de pacote e descrição do pacote estão contidos no elemento `<tr>` do HTML. Para selecionar todos os textos contidos nesse elemento, basta executar `response.css("tr ::text").getall()`. Após a sua execução, tem-se uma lista de strings com as informações de todos os pacotes. No entanto, nessa lista vem algumas strings vazias, que deverão ser removidas. O código a seguir mostrará todos os seletores que já foram utilizados no shell mas agora inclusos no spider para iniciar com o processo de raspagem dos dados:
...
class PackagescraperSpider(scrapy.Spider):
    ...
    def parse_packages_info(self, response):
        ...
        packages_element = [
          x.strip() for x in get_all_tr if x.strip() != ""
        ][3:]
...
Esse código possui a mesma classe mostrado anteriormente no `packagescraper.py`. No entanto, agora ele contém os seletores que foram analisados pelo shell, um novo método e algumas outras alterações. Por exemplo, o método `parse(self, response)`, mostrada anteriormente, é a responsável por capturar o link que leva à tabela de pacotes. Esse link é capturado através do seletor definido anteriomente no shell. Em seguida, esse link é concatenado à url base para formar a url completa que será seguida pelo spider. O método `response.follow()` é utilizado para acessar essa nova página e chamar o método `parse_packages_info` para continuar o processo de raspagem.

\vspace{12pt}

Dentro do método `parse_packages_info`, todos os textos contidos nos elementos `<tr>` são coletados com `response.css("tr ::text").getall()`. Como essa extração pode incluir strings vazias, é aplicado um strip() em cada elemento da lista e removidos aqueles que resultam em uma string vazia. Além disso, os primeiros três elementos são ignorados, pois não contêm informações relevantes.
...
Com esse código, os dados coletados podem ser salvos em diferentes formatos, como `.json` ou `.csv`, ao executar o spider. Por exemplo, para salvar os dados como `.csv`, basta executar no terminal o comando `scrapy crawl -o packages.csv packagescraper`. Ao executar o comando, ficará salvo um arquico com nome `packages.csv`, o qual conterá todos os dados coletados do site alvo.

\vspace{12pt}

O exemplo dado aqui é bastante simples. No entanto, a depender das necessidades do projeto, deverão ser utilizadas outras ferramentas do [**Scrapy**](https://docs.scrapy.org/en/latest). O código completo da raspagem dos dados de imóveis podem ser acessados em <https://github.com/cowvin0/tcc_realestate/tree/main/scrapy_zap>. Esse código pode ser utilizado como material complementar ao que foi apresentado nesse texto, já que foram adicionadas mais funcionalidades devido a necessidade durante a coleta dos dados.

### Exemplo de web scraping em R

\ \ \ Agora que já é conhecida a estrutura e já foi feita a inspeção do que se deseja coletar do site através das ferramentas de desenvolvedor do Firefox, o exemplo em **R**  será mais direto. Neste exemplo, foram utilizadas apenas o pacote [**rvest**](https://rvest.tidyverse.org/), [**dplyr**](https://dplyr.tidyverse.org/) para a criação da tabela com as informações coletadas e o [**purrr**](https://purrr.tidyverse.org/), que é um pacote de **R** para programação funcional. O código criado para extração pode ser visualizado abaixo:
...
Após carregar os pacotes necessários, o código define a url base do CRAN e acessa a página inicial que contém inúmeras informações sobre os pacotes de **R**. Em seguida, o código extrai o primeiro link do href presente na página e constrói a url completa para acessar a página com os pacotes ordenados por data de publicação.

\vspace{12pt}

Depois de carregar o conteúdo da página, o código busca as informações contidas nos elementos `<tr>` da tabela exibida no site. Os textos são extraídos e processados para remover o cabeçalho, e então são divididos em colunas separadas usando a função `strsplit()`, que vai separar as informações de data de publicação, nome do pacote e descrição do pacote.

\vspace{12pt}

Por fim, os dados são organizados em um `tibble`, onde cada linha contém a data de publicação, o nome do pacote e sua descrição. O uso das funções `map_chr` do pacote purrr garante que os valores sejam extraídos retornando vetores de caracteres com cada uma das informações.

\vspace{12pt}

Assim como no exemplo dado em **Python**, o projeto de raspagem com as necessidades e informações necessárias para serem coletadas durante a coleta dos dados. Ou seja, embora os exemplos dados acima tenham sido bastante simples, a depender do site, poderiam ser adicionadas outras funcionalidades como clicar em botões, descer página automaticamente para gerar o conteúdo e muitas outras coisas.

\vspace{12pt}

Portanto, chega ao fim descrição das tecnologias utilizadas para a obtenção dos dados e os exemplos de raspagem em **R** e **Python**. No entanto, a linguagem Python, em particular, continuou sendo empregada em outras etapas deste trabalho, como na modelagem dos valores de imóveis. Dessa forma, os próximos recursos apresentados serão aqueles utilizados especificamente para a modelagem e visualização dos resultados.
