...
Além disso, de acordo com @lundberg2018consistent, o SHAP possui três propriedades: Local accuracy, Missingness e Consistency. A propriedade de Local accuracy diz que a soma das contribuições das covariáveis $\left( \phi_ j \right)$ é igual ao valor da predição do modelo que se deseja explicar. O Missingness diz que covariáveis ausentes no vetor de coalizão $\left(\text{tal que } z_{j}^{'} = 0\right)$ não são atribuídas nenhuma importância, ou seja $\phi_j = 0$.
