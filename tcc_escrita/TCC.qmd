...
Com a execução do comando, tem-se uma string com nome 'available_packages_by_date.html'. É a partir dessa string que será feita a paginação. O `index.html` que está no atributo `start_urls` deverá ser substuído pela string obtida e seguir para a página de pacotes ordenados por data de publicação. Agora, com esse href, pode-se utilizar o shell e executar o `fetch` na url <https://cran.r-project.org/web/packages/available_packages_by_date.html> para carregar o conteúdo da página com a tabela de todos os pacotes ordenados por data.
