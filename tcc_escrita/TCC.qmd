...
\ \ \ O algoritmo de Gradient Boosting é semelhante ao Boosting, mas com algumas diferenças. Ele constrói modelos aditivos ajustando sequencialmente funções base aos pseudos-resíduos em cada iteração, que, por sua vez, correspondem a direção contrária dos gradientes da função de perda [@friedman2002stochastic]. Esses pseudos-resíduos indicam a direção na qual a função de perda está sendo minimizada, orientando o ajuste dos modelos seguintes.
...
A distribuição das variáveis foi analisada em termos do tipo do imóvel a partir de um gráfico de violino. Pela @fig-violin, é fácil perceber que a maioria das distribuições possuem assimetria negativa. As distribuições possuem caudas longas à direita, indicando presença de valores extremamente altos, indicando a necessidade da aplicação de alguma transformação para a estabilização da variância.
