...
\ \ \ Matematicamente, o Stacking define predições $\hat{f}_m^{-i}\left(x\right)$ em x, utilizando o modelo estatístico ou algoritmo $m$, aplicado ao conjunto de treinamento com a $i-ésima$ observação removida [@hastie2009elements]. Assim, os peso são estimados da seguinte forma

$$
\hat{w}^{st} = \arg \min_{w} \sum^{N}_{i = 1}
$$
