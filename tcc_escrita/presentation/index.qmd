...
- **Transformação das variáveis numéricas:**
  + Para estabilizar a variância e tornar a distribuição das variáveis independentes mais próximas de uma normal, foram aplicadas duas transformações:
    + Foi aplicada a transformação de $\log\left(1+x\right)$ nas variáveis na maioria das variáveis numéricas, com exceção da variável do produto entre as coordenadas geográficas e o preço médio do aluguel no bairro, valor e área média do aluguel no bairro.
...
![Validação cruzada K-Fold.](cv_test_imagem.png){#fig-kfold fig-align="center"}
:::
...
- A função $\hat{a}_{U}\left(\boldsymbol{\theta}_U\right)$ representa uma estimativa da performance marginal do algoritmo $A$ em relação ao subconjunto $U$ dos hiperparâmetros, sendo definida por:
  $$
  \hat{a}_{U}\left(\boldsymbol{\theta}_U\right) = \frac{1}{\|\boldsymbol{\Theta}_{T}\|} \int \hat{y}\left(\boldsymbol{\theta}_{N|U}\right)d \boldsymbol{\theta}_{T}\text{.}
  $$
...
- A interpretação dos valores de Shapley é que a j-ésima covariável contribui $\phi$ para a predição de uma determinada instância.
...
- Melhor desempenho geral — Gradient Boosting:
  ...
  + Também apresentou o maior $R^2$ (87,33%), o que sugere maior capacidade explicativa em relação à variabilidade dos dados.
...
- Uso de técnicas automatizadas para extrair dados de sites de anúncios.

- Construção de uma base de dados abrangente e rica em variáveis.

- Modelagem com Aprendizado de Máquina
  - Gradient Boosting como o modelo com melhor desempenho:
    + RMSE: 0,28297
    + MAPE: 1,333%
    + R2R2: 87,33%

- Interpretação do Modelo
  + Técnicas utilizadas: SHAP, ICE e PDP.
  + Resultados importantes:
    + Variáveis como área, número de quartos, banheiros e aluguel → impacto direto nas predições.

    + Variáveis com impacto limitado (ex.: área de aluguel), mas ainda relevantes em casos específicos (ex.: sauna, spa em imóveis de alto padrão).

- Variáveis de Maior e Menor Impacto (SHAP)
  + Maior impacto: área do imóvel, razão quartos/área, coordenadas geográficas.
  + Menor impacto: sauna, spa, portaria, quadra de esportes.
