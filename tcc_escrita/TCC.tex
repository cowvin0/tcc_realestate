...
\makeatother
\makeatletter
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[boxrule=0pt, sharp corners, enhanced, breakable, colback={codebgcolor}, frame hidden]}{\end{tcolorbox}}\fi
\makeatother
\makeatletter
...
por sua vez, correspondem aos gradientes da função de perda (FRIEDMAN,
2002). Esses podem gradientes indicam a direção na qual a função perda
diminui mais rapidamente, orientando o ajuste dos modelos seguintes.
...
Os pseudo-resíduos \(r_{im}\)\hspace{0pt} representam os gradientes da
função de perda \(L\), conforme expressos na linha 2(a) do
 Algoritmo~\ref{algo-gradboos} , e indicam a direção de maior diminuição
de \(L\).

\vspace{12pt}

Assim, cada árvore de regressão é ajustada aos novos pseudo-resíduos,
com o objetivo de minimizar o erro residual acumulado pelas árvores
anteriores. O estimador é, então, atualizado separadamente em cada
região \(R_{jm}\)\hspace{0pt}, conforme descrito a seguir:

\[
f_m\left(x\right) = f_{m - 1}\left(x\right) + \lambda \sum^{J_m}_{j = 1} \gamma_{jm} I\left(x \in R_{jm}\right)\text{,}
\] em que
\(\gamma_{jm} = \arg \min_{\gamma} \sum_{x_i \in R_{jm}} L\left(y_i, f_{m - 1}\left(x_i\right) + \gamma\right)\)
representa a constante ótima para cada região. O parâmetro \(\lambda\),
assim como no algoritmo de boosting, é o hiperparâmetro de shrinkage,
utilizado para controlar a taxa de aprendizado. Valores menores de
\(\lambda\) exigem um número maior de iterações \(M\).
...
presença de valores extremamente altos e que indicam que a necessidade
da aplicação de alguma transformação para a estabilização da variância.
...
a tunagem da quantidade máxima do número de folhas das árvores. O espaço
de busca para a quantidade de árvores foi definido entre 100 e 2000,
para a taxa de aprendizado no mesmo intervalo usado no Gradient
Boosting, para a profundidade das árvores e número de folhas o intervalo
foi definido entre 100 e 600
