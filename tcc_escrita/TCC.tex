...
assumindo que \(y^* > y\), considera-se a parametrização de
\(p\left(\lambda,y\right)\) como
\(p\left(y\right)p\left(\lambda∣y\right)\), com o objetivo de
simplificar os cálculos. Dessa forma, a expressão do EI é reduzida para:
...
distribuição marginal \(p\left(\lambda\right)\). Portanto, segue-se que:
...
Dessa forma, agora basta calcular a integral
\(\int_{\infty}^{y^*}\left(y^* - y\right) p\left(\lambda|y\right)p\left(y\right)dy\).
Assim,
\(l\left(\lambda\right) \int_{\infty}^{y^*}\left(y^* - y\right)p\left(y\right)dy = \gamma y^{*} l\left(\lambda\right) - l\left(\lambda\right) \int_{-\infty}^{y^*} y p\left(y\right)dy\).
Finalmente, substituindo as expressões anteriores na
Equação~\ref{eq-exp}, tem-se que:
