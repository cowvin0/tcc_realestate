...
\makeatother
\makeatletter
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[sharp corners, boxrule=0pt, enhanced, colback={codebgcolor}, frame hidden, breakable]}{\end{tcolorbox}}\fi
\makeatother
\makeatletter
...
valores estimados dos imóveis estão, em média, \(1,333%
\) dos seus valores observados. A relação entre os valores estimados e
os observados pode ser visualizada na Figura~\ref{fig-preds_gdt}.
...
em concordância com a maioria das curvas ICE. Para a quantidade de
banheiros, observa-se uma relação majoritariamente estável, com apenas
pequenas variações, comportamento semelhante ao verificado para o número
de quartos, o que sugere uma influência limitada dessas variáveis no
modelo. No entanto, embora essas relações sejam bastante estáveis,
observa-se um aumento nas predições do modelo com o aumento da
quantidade de banheiro e quartos
...
evidenciando uma influência positiva dessa variável sobre o valor dos
imóveis.
...
algoritmos baseados em árvores, que se baseiam na redução da impureza ou
no ganho de informação ao longo das divisões do modelo, o SHAP calcula a
contribuição de cada variável com base na magnitude dos valores de
Shapley atribuídos individualmente às predições.
...
predição do modelo. Além disso, ao utilizar a biblioteca
\href{https://shap.readthedocs.io/en/latest}{\textbf{SHAP}} para gerar
os gráficos apresentados na Figura~\ref{fig-shap_importance} e na
Figura~\ref{fig-shap_summary}, há um limite de variáveis incluídas nos
gráficos e as variáveis são ordenadas de acordo com seu impacto na
predição do modelo. Dessa forma, variáveis como sauna, spa, portaria 24
horas e quadra de esporte acabam sendo excluídas dessas visualizações
por serem as que apresentaram o menor impacto na predição do modelo.
