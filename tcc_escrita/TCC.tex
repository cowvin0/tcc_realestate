...
\makeatother
\makeatletter
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[frame hidden, colback={codebgcolor}, breakable, sharp corners, enhanced, boxrule=0pt]}{\end{tcolorbox}}\fi
\makeatother
\makeatletter
...
Na sessão a seguir, apresenta-se um exemplo de como realizar web
scraping em \textbf{R} e \textbf{Python}. Para isso, utilizam-se os
...
biblioteca empregada para esse fim. Além disso, essa sessão servirá
também para mostrar a estruturar de um HTML e o funcionamento dos
seletores para selecionar elementos de um HTML. É importante ressaltar
que os códigos desenvolvidos para a raspagem de dados podem deixar de
funcionar ao longo do tempo devido a possíveis mudanças na estrutura dos
sites. Assim, eventuais ajustes no código poderão ser necessários.
...
desses dados, será gerado um arquivo \texttt{.csv} contendo essas
informações.
...
dependências, pode-se instalar diretamente utilizando o
\href{https://pip.pypa.io/en/stable/}{\textbf{pip}}, que é o gerenciador
de pacotes para \textbf{Python}. Portanto, para isso, basta executar em
seu terminal o comando \texttt{pip\ install\ Scrapy}.

\vspace{12pt}

Após a instalação do
\href{https://docs.scrapy.org/en/latest}{\textbf{Scrapy}}, ficará
disponível o uso de seu CLI\footnote{CLI (Command Line Interface) é uma
  interface de linha de comando que permite a interação com um sistema
  operacional ou software por meio de comandos digitados em um terminal,
  sem o uso de interface gráfica.}. O CLI disponibilizará inúmeras
funcionalidades para o
\href{https://docs.scrapy.org/en/latest}{\textbf{Scrapy}}, tais como
inspencionar o conteúdo da página para encontrar mais facilmente os
seletores daquela página em específico e, o mais importante, iniciar um
projeto \href{https://docs.scrapy.org/en/latest}{\textbf{Scrapy}}.

\vspace{12pt}

Antes de se iniciar a raspagem dos dados com o
\href{https://docs.scrapy.org/en/latest}{\textbf{Scrapy}}, é necessário
iniciar um projeto utilizando o seu CLI. Para isso, basta executar o
comando
\texttt{scrapy\ startproject\ \textless{}nome\_do\_projeto\textgreater{}}.
Esse comando criará um diretório com todos os arquivos e configurações
iniciais necessários para começar a extração de dados com o
...
será criada um diretório com a seguinte estrutura:
...
Com a estrutura do projeto criada, é possível criar o primeiro spider do
projeto. Para isso, é utilizado o comando
\texttt{scrapy\ genspider\ \textless{}nome\_do\_spider\textgreater{}\ \textless{}website\textgreater{}}.
Neste exemplo, serão raspados informações do site
...
executado anteriormente. Essa classe possui três atributos e um método.
O atributo \texttt{name} nomeia o spider e vai ser utilizado
posteriormente para inicializar a raspagem dos dados. O
\texttt{allowed\_domains} é o atributo que indica para o
\href{https://docs.scrapy.org/en/latest}{\textbf{Scrapy}} que ele deve
apenas raspar informações que estão naquele domínio, nesse caso,
\texttt{cran.r-project.org}. O atributo \texttt{start\_urls} indica qual
deve ser a primeira url a ser raspada. Por último, o método
\texttt{parse} é executada depois que uma resposta foi obtida após a
requisição no site alvo.

\vspace{12pt}

Para extrair dados do conteúdo HTML da página, será necessário utilizar
os seletores \texttt{CSS} ou \texttt{XPath}. Nesse caso, será utilizado
o seletor do \texttt{CSS}. De forma a facilitar a seleção de conteúdo da
página, será utilizado uma das grandes ferramentas do
\href{https://docs.scrapy.org/en/latest}{\textbf{Scrapy}}, que é o
\href{https://docs.scrapy.org/en/latest/topics/shell.html}{\textbf{Scrapy
Shell}}. Essa ferramenta ajudará a selecionar as informações que
necessárias e até mesmo a debugar o código. Para executar o
\href{https://docs.scrapy.org/en/latest/topics/shell.html}{\textbf{Scrapy
...
configurações gerais que já estão sendo utilizadas no seu projeto.
Agora, com o shell ativado, basta trazer todo o conteúdo HTML contido na
página de interesse com o seguinte comando:
...
\caption{\label{fig-pages_scrape}A Figura~\ref{fig-page_contents} mostra
a página que está sendo raspada, enquanto a
Figura~\ref{fig-page_contents_devtools} mostra a mesma página mas com um
exemplo do uso das ferramentas de desenvolvedor do Firefox.}

\end{figure}%

Com a execução do comando anterior, o conteúdo da página na
Figura~\ref{fig-page_contents} será gerado e o conteúdo contido em seu
HTML poderá ser extraído. Se o \texttt{fetch} deu certo, após a execução
do comando deverá aparecer um código 200, que é um código de status HTTP
que indica que uma requisição foi bem sucedida. A saída a seguir
...
salvo em um objeto \texttt{response}, que faz parte nativamente do
shell. Por exemplo, caso se deseja obter o conteúdo contido no HTML da
página, basta executar \texttt{response.body}. É a partir dessa resposta
que o conteúdo da página poderá ser analisado e os seletores poderão
finalmente ser configurados. No entanto, para facilitar ainda mais esse
trabalho, alguns navegadores possuem ferramentas de desenvolvedor, que
facilitam a inspeção da estrutura da página, como pode ser visto na
Figura~\ref{fig-page_contents_devtools}.
...
Após inspencionar a página, percebe-se que possuem alguns elementos
\texttt{\textless{}a\textgreater{}} no corpo do HTML e que possuem um
href relacionado a eles. Esses href serão necessários para fazer a
paginação fazer a coleta dos pacotes ordenados por data de publicação.
Para extrair esses href, basta executar em seu terminal o comando
\texttt{response.css("a{[}target=\_top{]}\ ::attr(href)").get()} que faz
uso dos seletores \texttt{CSS}. Esse comando é usado para extrair apenas
o primeiro href de elemento \texttt{\textless{}a\textgreater{}} que
possuem o atributo target=``\_top''. Caso fosse necessário extrair todos
os href, bastaria substituir o método \texttt{.get} por
\texttt{.getall()}.

\vspace{12pt}

Com a execução do comando, é extraída uma string que servirá para fazer
a paginação, com nome
\texttt{\textquotesingle{}available\_packages\_by\_date.html\textquotesingle{}}.
É a partir dessa string que será feita a paginação. O
\texttt{index.html} que está no atributo \texttt{start\_urls} deverá ser
substituído pela string obtida e seguir para a págian de pacotes
ordenados por data de publicação. Agora, com esse href, pode-se utilizar
o shell e executar o \texttt{fetch} na url
...
A nova página contém uma tabela geral de todos os pacotes ordenados por
data e contendo a sua descrição. Cada data, nome de pacote e descrição
do pacote estão contidos no elemento
\texttt{\textless{}tr\textgreater{}} do HTML. Para selecionar todos os
textos contidos nesse elemento, basta executar
\texttt{response.css("tr\ ::text").getall()}. Após a sua execução,
tem-se uma lista de strings com as informações de todos os pacotes. No
entanto, nessa lista vem algumas strings vazias, que deverão ser
removidas. O código a seguir mostrará todos os seletores que já foram
utilizados no shell mas agora inclusos no spider para iniciar com o
processo de raspagem dos dados:
...
\NormalTok{          x.strip() }\ControlFlowTok{for}\NormalTok{ x }\KeywordTok{in}\NormalTok{ get\_all\_tr }\ControlFlowTok{if}\NormalTok{ x.strip() }\OperatorTok{!=} \StringTok{""}
...
Esse código possui a mesma classe mostrado anteriormente no
\texttt{packagescraper.py}. No entanto, agora ele contém os seletores
que foram analisados pelo shell, um novo método e algumas outras
alterações. Por exemplo, o método \texttt{parse(self,\ response)},
mostrada anteriormente, é a responsável por capturar o link que leva à
tabela de pacotes. Esse link é capturado através do seletor definido
anteriomente no shell. Em seguida, esse link é concatenado à url base
para formar a url completa que será seguida pelo spider. O método
\texttt{response.follow()} é utilizado para acessar essa nova página e
chamar o método \texttt{parse\_packages\_info} para continuar o processo
de raspagem.
...
extração pode incluir strings vazias, é aplicado um strip() em cada
elemento da lista e removidos aqueles que resultam em uma string vazia.
Além disso, os primeiros três elementos são ignorados, pois não contêm
informações relevantes.

\vspace{12pt}

Após a limpeza, os dados são organizados em três listas: pub\_date,
packs\_name e desc\_packs. Cada uma delas recebe elementos específicos
da lista filtrada, onde, \texttt{pub\_date} contém as datas de
publicação dos pacotes, \texttt{packs\_name} contém os nomes dos pacotes
e \texttt{desc\_packs} contém as descrições dos pacotes.
...
terminal o comando
\texttt{scrapy\ crawl\ -o\ packages.csv\ packagescraper}. Ao executar o
comando, ficará salvo um arquico com nome \texttt{packages.csv}, o qual
conterá todos os dados coletados do site alvo.

\vspace{12pt}

O exemplo dado aqui é bastante simples. No entanto, a depender das
necessidades do projeto, deverão ser utilizadas outras ferramentas do
\href{https://docs.scrapy.org/en/latest}{\textbf{Scrapy}}. O código
completo da raspagem dos dados de imóveis podem ser acessados em
\url{https://github.com/cowvin0/tcc_realestate/tree/main/scrapy_zap}.
Esse código pode ser utilizado como material complementar ao que foi
apresentado nesse texto, já que foram adicionadas mais funcionalidades
devido a necessidade durante a coleta dos dados.

\subsection{Exemplo de web scraping em
R}\label{exemplo-de-web-scraping-em-r}

~~~Agora que já é conhecida a estrutura e já foi feita a inspeção do que
se deseja coletar do site através das ferramentas de desenvolvedor do
Firefox, o exemplo em \textbf{R} será mais direto. Neste exemplo, foram
utilizadas apenas o pacote
\href{https://rvest.tidyverse.org/}{\textbf{rvest}},
\href{https://dplyr.tidyverse.org/}{\textbf{dplyr}} para a criação da
tabela com as informações coletadas e o
\href{https://purrr.tidyverse.org/}{\textbf{purrr}}, que é um pacote de
\textbf{R} para programação funcional. O código criado para extração
...
e acessa a página inicial que contém inúmeras informações sobre os
pacotes de \textbf{R}. Em seguida, o código extrai o primeiro link do
href presente na página e constrói a url completa para acessar a página
com os pacotes ordenados por data de publicação.
...
cabeçalho, e então são divididos em colunas separadas usando a função
\texttt{strsplit()}, que vai separar as informações de data de
publicação, nome do pacote e descrição do pacote.

\vspace{12pt}

Por fim, os dados são organizados em um \texttt{tibble}, onde cada linha
contém a data de publicação, o nome do pacote e sua descrição. O uso das
funções \texttt{map\_chr} do pacote purrr garante que os valores sejam
extraídos retornando vetores de caracteres com cada uma das informações.

\vspace{12pt}

Assim como no exemplo dado em \textbf{Python}, o projeto de raspagem com
as necessidades e informações necessárias para serem coletadas durante a
coleta dos dados. Ou seja, embora os exemplos dados acima tenham sido
bastante simples, a depender do site, poderiam ser adicionadas outras
funcionalidades como clicar em botões, descer página automaticamente
para gerar o conteúdo e muitas outras coisas.

\vspace{12pt}

Portanto, chega ao fim descrição das tecnologias utilizadas para a
obtenção dos dados e os exemplos de raspagem em \textbf{R} e
\textbf{Python}. No entanto, a linguagem Python, em particular,
