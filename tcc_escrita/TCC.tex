...
\makeatother
\makeatletter
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[sharp corners, colback={codebgcolor}, breakable, boxrule=0pt, enhanced, frame hidden]}{\end{tcolorbox}}\fi
\makeatother
\makeatletter
...
Com a execução do comando, tem-se uma string com nome
`available\_packages\_by\_date.html'. É a partir dessa string que será
feita a paginação. O \texttt{index.html} que está no atributo
\texttt{start\_urls} deverá ser substuído pela string obtida e seguir
para a página de pacotes ordenados por data de publicação. Agora, com
esse href, pode-se utilizar o shell e executar o \texttt{fetch} na url
